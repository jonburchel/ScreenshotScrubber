...
var typingTimer;
var doneTypingInterval = 750; // wait 750ms after typing stops to save changed values...
function doneTyping()
{
    StoreConfigValues();
}
...
function AddRow()
{
    var row = SettingsList.insertRow(SettingsList.rows.length - 1);
    var id = CreateGUID();
    row.id = "Row" + id;
...
{
    AddRowButton.hidden = false;
    var ReplaceValues = new Array(SettingsList.rows.length - 2);
...
function DataChanged(e)
{
    ...
    let DeleteRowButton = document.getElementById("Delete" + id);
    var i = 0;
    for (i = 1; i < SettingsList.rows.length - 1; i++) 
    {
    ...
    if (i == SettingsList.rows.length - 1)
    {    
